...
public class IncomingInvitationActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String user = getIntent().getStringExtra(Constants.KEY_NAME);

        if (user != null) {
            textFirstChar.setText(user.substring(0, 1));
        }
        textUsername.setText(String.format("%s", user,getIntent().getStringExtra(Constants.KEY_NAME)));
    ...
    private void sendRemoteMessage(String remoteMessageBody, String type) {
        ApiClient.getClient().create(ApiService.class)
                .sendRemoteMessage(Constants.getRemoteMsgHeaders(), remoteMessageBody)
                .enqueue(new Callback<String>() {
                    @Override
                    public void onResponse(@NonNull Call<String> call, @NonNull Response<String> response) {
                        if (response.isSuccessful()) {
                            if (type.equals(Constants.REMOTE_MSG_INVITATION_ACCEPTED)) {
                                Toast.makeText(IncomingInvitationActivity.this,"Invite Accept",Toast.LENGTH_SHORT).show();
                                try {
